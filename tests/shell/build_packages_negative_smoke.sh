...
if release/build-packages.sh --allow-dirty \
  --out-dir "${WORK_DIR}/stale" v0.3.0 >"${WORK_DIR}/stale.log" 2>&1; then
...
if release/build-packages.sh --allow-dirty \
  --out-dir "${WORK_DIR}/linked-output" v0.3.0 >"${WORK_DIR}/symlink.log" 2>&1; then
...
if release/build-packages.sh --allow-dirty \
  --out-dir "${WORK_DIR}/bad-tag" 0.3.0 >"${WORK_DIR}/tag.log" 2>&1; then
