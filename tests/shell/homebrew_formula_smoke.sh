...
cd "${ROOT}"
scripts/build-pgxn-dist.sh --allow-dirty --out-dir "${WORK_DIR}/dist" v0.2.0
scripts/render-homebrew-formula.sh \
  --archive "${WORK_DIR}/dist/pgContext-0.2.0.zip" \
  --out-dir "${WORK_DIR}/Formula"

formula="${WORK_DIR}/Formula/pgcontext.rb"
grep -qF 'releases/download/v0.2.0/pgContext-0.2.0.zip' "${formula}"
