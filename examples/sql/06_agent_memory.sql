-- Agent memory over ordinary PostgreSQL tables.
--
-- Demonstrates durable decision memory for an agent principal:
--   1. Register a collection on decision rows with vectors + metadata filters.
--   2. Run hybrid dense + full-text retrieval for a memory-style question.
--   3. Hydrate ranked hits into a compact context pack for downstream LLM use.
--
-- Fixture embeddings are 4-dimensional so the script runs without an external
-- embedding model. Replace query literals with vectors from your application model.
...
-- Simulated agent question:
-- "What billing decisions did Alice make previously?"
-- Application code would embed the question; this script uses a fixture vector.
