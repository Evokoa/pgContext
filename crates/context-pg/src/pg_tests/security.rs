...
fn security_definer_catalog_writers_ignore_hostile_shadow_objects() {
    ...
    Spi::run(
        "CREATE INDEX msec_catalog_docs_hnsw ON public.msec_catalog_docs
             USING pgcontext_hnsw (embedding pgcontext.vector_hnsw_ops)",
    )
    .expect("profile index should be created");
    ...
    for (profile, revision, hash) in [
        ("profile_v1", "v1", "0123456789abcdef"),
        ("profile_v2", "v2", "fedcba9876543210"),
    ] {
        Spi::run(&format!(
            "SELECT pgcontext.register_embedding_profile(
                 'msec_catalog_docs', '{profile}', 'embedding',
                 'public.msec_catalog_docs_hnsw',
                 jsonb_build_object(
                     'representation', 'dense', 'dimensions', 2,
                     'normalization', 'none', 'metric', 'l2',
                     'provider', 'fixture', 'model', 'model', 'revision', '{revision}',
                     'input_template', '{{t}}', 'output_template', '{{v}}',
                     'bit_order', NULL, 'byte_order', NULL, 'scale', NULL,
                     'zero_point', NULL, 'configuration_hash', '{hash}'
                 )
             )"
        ))
        .expect("embedding profile should ignore shadow catalog");
    }
    Spi::run(
...
fn security_shadow_catalog_row_count(schema_name: &str) -> i64 {
    security_count(&format!(
        "SELECT (
            ...
            (SELECT count(*) FROM {schema_name}._embedding_profiles) +
