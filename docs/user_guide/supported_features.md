...
| Bulk point maintenance <!-- capability:CAP-BULK-POINTS --> | Stable | Bulk upsert, delete, and source-table backfill APIs process mappings in bounded, idempotent batches and report progress. |
...
| Experimental build-job metadata | Experimental | Owner-scoped rows track bounded synchronous segment or mmap work, progress, cancellation, abandonment, retry, and terminal state; they do not make `CREATE INDEX` resumable. |
