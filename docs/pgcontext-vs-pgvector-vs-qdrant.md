...
| Sparse vectors | **Experimental** SQL type and HNSW format; exact search plus stable L2/IP/cosine/L1 opclass names | **Native** `sparsevec` and HNSW | [**Native** sparse vectors and sparse index](https://qdrant.tech/documentation/manage-data/vectors/#sparse-vectors) | Named sparse ANN remains a pgContext roadmap item; Qdrant and pgvector are more mature for sparse serving. |
